...
int main(int argc, char **argv)
{
	...
	CNN_GPU<<<256, 256>>>(matrix_g, G_g_result, kernel2_G, SIZE, 256, 25);
	//printf("Came In3\n");
	cudaEventRecord(c_stop,0);
	cudaEventSynchronize(c_stop);
	cudaEventElapsedTime(&GTime, c_start, c_stop);
	cudaMemcpy(g_result, G_g_result, sizeof(int)*(SIZE-4)*(SIZE-4), cudaMemcpyDeviceToHost);
	printf("GPU_gaussian %lf s \n",  (double)GTime/1000);
	FILE* write_ptr2;
	write_ptr2 = fopen("out_g.bin", "wb");
	...
	int j = blockDim.x * blockIdx.x + threadIdx.x;
    	for(int i=0;i<bb;i+=TOTAL)
    	{
			...
			for (int k = 0; k < big; k++)
			{
				...
				t = (t + kernel[k] * m1[(k /temp_B + (i+j)/ bias) * SIZE + k % temp_B + (i+j) % bias]);
